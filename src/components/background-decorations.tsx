  ...
  useEffect(() => {
    const generateShapes = () => {
      const newShapes = Array.from({ length: 10 }).map((_, i) => {
        ...
        const color = `hsla(var(--foreground), ${Math.random() * 0.1 + 0.05})`;
  ...
  return (
    <div className="fixed inset-0 -z-10 overflow-hidden" aria-hidden="true">
       <div className="absolute inset-0 w-full h-full opacity-20" style={{
          backgroundImage: 'linear-gradient(hsl(var(--neon-blue)/0.15) 1px, transparent 1px), linear-gradient(to right, hsl(var(--neon-blue)/0.15) 1px, transparent 1px)',
